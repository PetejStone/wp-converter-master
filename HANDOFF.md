...
15. **Nav containers are matched by hardcoded Scorpion element IDs.** `pipeline/build/templates.ts` finds the nav regions to swap for dynamic `wp_nav_menu` renders by literal id: `#HeaderS4TopNav` (desktop primary), `#HeaderS4MobileNav` (mobile drawer), `#FooterS3Nav` (footer Quick Links), and `#SideNavV1SideNav` (branch side nav). `navigation.ts` also keys footer extraction off `#FooterS3Nav`. The `S4` / `S3` / `V1` suffixes are Scorpion **section-module version** identifiers — they change when a site uses a different header/footer module or USC version — so on any site that doesn't use those exact modules the swap silently misses and the page ships a **static, non-editable** nav (the desktop primary still works because the crawler already captures it via the semantic `[role="menubar"]` selector, but `templates.ts` is now inconsistent with that). **Fix:** replace the id lookups with version-independent structural/semantic selectors — desktop is `ul[role="menubar"]` (already proven in `crawl-page.ts`), mobile is likely `ul[role="menu"][aria-label="Mobile Navigation"]` / `nav.mbl-nv` / `nav[data-role="panel"]`, footer + side nav need their structures confirmed. Jesse to provide the `/wp-converter/` nav page structure (roles, aria-labels, data-attrs, stable classes, and whether they're consistent across USC 3.0/4.0/4.2) so we can pick the most robust anchor for each, with safe fallbacks. Align `templates.ts` and `navigation.ts` on the same selectors. **Deferred — tackle later.**
